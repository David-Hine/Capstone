...
var serviceArray = new Array(5);
var NUScharacArray = new Array(2); //[0]: SAADC char [1]: UART TX notification [2]: Battery notification, 
var SAADCcharac;


function readServices(){
	console.log(SAADCcharac.uuid);
	console.log('Press any key to toggle notifications on the SAADC characteristic. ' +
		...
		addUserInputListener(adapter, NUScharacArray[0]);
}
 ...
 *
 * @param {Adapter} adapter Adapter being used.
 * @param {Service} Service The SAADC service to discover characteristics from
 * @param {Service} UUID of characteristic
 * @returns {Promise} Resolves on successfully discovering the SAADC measurement characteristic.
 *                    If an error occurs, rejects with the corresponding error.
 */
function discoverCharacteristic(adapter, Service, UUID, index) {
    return new Promise((resolve, reject) => {
        adapter.getCharacteristics(Service.instanceId, (err, characteristics) => {
            ...
            for (const characteristic in characteristics) {
                if (characteristics[characteristic].uuid === UUID) {
					...
					//add char to array
					characArray[index]=characteristics[characteristic];
...
function addUserInputListener(adapter, charID) {
    ...
    process.stdin.on('readable', () => {
        ...
        if (chunk[0] === 'q' || chunk[0] === 'Q') {
            ...
            if (notificationsEnabled[0]) {
				...
				//start adc
				adapter.writeCharacteristicValue(charID.instanceId, [0x01, 0x00], false, err => {
                if (err) {
                    console.log(`Error enabling writing to ADC: ${err}.`);
                    process.exit(1);
                }
				...
				/*adapter.startCharacteristicsNotifications(characArray[0].instanceId, false, err => {
                if (err) {
...
function addAdapterListener(adapter) {
    ...
    adapter.on('deviceConnected', device => {
		...
		adapter.getServices(device.instanceId, (err, services) => {
			...
			for (const service in services) {
				serviceArray[service] = services[service];
                console.log(serviceArray[service].uuid);
				
				//put NUS chars NUScharacArray
				if(serviceArray[service].uuid == BLE_UUID_NUS_SREVICE){
					console.log('Found NUS char');
					adapter.getCharacteristics(serviceArray[service]._instanceId, (err, characteristics) => {
						if (err) {
							reject(Error(`Error discovering the SAADC's characteristics: ${err}.`));
							return;
						}
						...
						//add to NUS characteristics
						
						for(const characteristic in characteristics){
							NUScharacArray[characteristic] = characteristics[characteristic];
							console.log(NUScharacArray[characteristic].uuid);
						}
					});
				...
				/*put SAADC char SAADCcharac
				if(serviceArray[service].uuid == BLE_UUID_SAADC_SERVICE_UUID){
					console.log('Found SAADC char');
					adapter.getCharacteristics(serviceArray[service]._instanceId, (err, characteristics) => {
						if (err) {
							reject(Error(`Error discovering the SAADC's characteristics: ${err}.`));
							return;
						...
						//add to SAADC characteristic variable
						for(const characteristic in characteristics){
							SAADCcharac = characteristics[characteristic];
							console.log(SAADCcharac.uuid);
							console.log('Got SAADC char');
						}
					});
		...
		//setTimeout(readServices(), 5000);
		
		
		console.log('Why is this at the beginning');
		print('TWO:' + NUScharacArray[0]);
		/*readServices();
		*/
		...
		discoverService(adapter, device, BLE_UUID_NUS_SREVICE).then(service => {
			...
			return discoverCharacteristic(adapter, service, BLE_UUID_NUS_TX_CHARACTERISTIC, 1)
            .then(characteristic => {
            ...
            .then(descriptor => {
				...
				//addUserInputListener(adapter, BLE_UUID_NUS_TX_CHARACTERISTIC);
            });
