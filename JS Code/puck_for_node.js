...
function interpret_data_and_save(acc_data, file_name){
	...
	var bat_level = acc_data[0];
	acc_data.splice(0,1);	//remove battery level
	
	var joined_values = acc_data.join();	//put all values into one variable
	var array_of_values = joined_values.split(','); 	//each element has a place in this array
	...
	var time = (x.length)/5000;		//time = (#samples per axis)/(5000 samples/sec)
	const time_interval = 0.0002;	//sampling at 5kHz, so 1/5000s = 0.0002s
	var current_time = 0;
	...
	for (var i=0;i<x.length; i++){
		...
		csvStream.write({Time: time_arr[i], X: x[i], Y: y[i], Z: z[i], Acceleration: resultant_acc[i]});
	}
