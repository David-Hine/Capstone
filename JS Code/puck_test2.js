...
//write to SAADC characteristic
function write_saadc_char(adapter, device){
	return new Promise((resolve, reject) => {
    discoverService(adapter, device, BLE_UUID_SAADC_SERVICE_UUID).then(service => {
            ...
            .then(descriptor => {
                console.log('Discovered the SAADC characteristic\'s CCCD.');
				
				//enable SAADC notifications
				adapter.writeDescriptorValue(descriptor.instanceId, [1, 0], false, err => {
					if (err) {
						//console.log(`Error enabling notifications on the hrm characteristic: ${err}.`);
						reject(Error(`Error enabling notifications on the SAADC characteristic: ${err}.`));
						process.exit(1);
					}

					console.log('Notifications enabled on the SAADC characteristic.');
					resolve('Done SAADC');
				});
				...
				adapter.addUserInputListener(adapter, characteristic);
				*/
...
function addUserInputListener(adapter, characteristic) {
    ...
    process.stdin.on('readable', () => {
        ...
        if (chunk[0] === 'q' || chunk[0] === 'Q') {
			...
			adapter.writeCharacteristicValue(characteristic, notificationsEnabled, false, err => {
                if (err) {
                    console.log(`Error enabling notifications on the hrm characteristic: ${err}.`);
                    process.exit(1);
                }
    ...
    adapter.on('deviceConnected', device => {
		...
		enable_bat_notif(adapter, device).then(function(){
			enable_nus_notif(adapter, device).then(function(){
				write_saadc_char(adapter, device);
			});
		});
		*/
